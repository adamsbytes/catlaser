...
- better-auth on the coordination server for sign-in with Apple, Google, and email magic links; native apps exchange provider ID tokens (or completed magic links) for a bearer token kept in platform secure storage (Keychain / EncryptedSharedPreferences); server fronted by Cloudflare Tunnel (no inbound ports on the origin), clients pin public roots in Cloudflare's edge certificate chain
- Magic-link delivery into the app via Universal Links (iOS) / App Links (Android); every authenticated request — sign-in ceremonies AND subsequent API calls — carries a hardware-key-signed attestation (Secure Enclave / Keystore) over the canonical device-fingerprint hash plus a per-call context binding (req:/ver:/sis:/out:/api:), server stores the per-session public key at sign-in and verifies each attestation; mutating routes additionally require a client idempotency key to block write-replay within the 60s skew window
